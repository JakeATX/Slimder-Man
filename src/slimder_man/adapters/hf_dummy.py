...
class DummyHfMoeLayer(nn.Module):
    def __init__(self, config: DummyHfMoeConfig):
        ...
        self.shared_experts = nn.ModuleList([DummyExpert(config.hidden_size, config.intermediate_size) for _ in range(config.num_shared_experts)])
...
class DummyTokenizer:
    ...
    def save_pretrained(self, output_dir: str | Path) -> tuple[str]:
        path = Path(output_dir)
        path.mkdir(parents=True, exist_ok=True)
        target = path / "tokenizer_config.json"
        target.write_text(json.dumps({"type": "dummy_hf_moe", "vocab_size": self.vocab_size}), encoding="utf-8")
        return (str(target),)
